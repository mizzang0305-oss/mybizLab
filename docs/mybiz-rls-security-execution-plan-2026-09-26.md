...
The scoped CI run [36210794714](https://github.com/mizzang0305-oss/mybizLab/actions/runs/36210794714) completed the draft migration, 21 RLS/role/cross-store assertions, anonymous Data API denial, 901 application tests, real public-store and merchant-orders handlers against persisted synthetic rows, and a synthetic data-preserving write lockdown. Onboarding setup-request and merchant order-event handler coverage is being added and needs a new CI result. Remaining work includes provisioning semantics and Auth binding compatibility, Production-specific schema parity, and a safe application rollback sequence. The emergency SQL draft is `supabase/migration_drafts/20260926_mybiz_public_rls_write_lockdown.sql`; it is not a data-drop rollback and has not been applied to Production.
