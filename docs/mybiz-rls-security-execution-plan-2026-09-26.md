...
The disposable CI run [36249128365](https://github.com/mizzang0305-oss/mybizLab/actions/runs/36249128365) passed on the code candidate with Production-like inquiry compatibility. The two SQL files remain drafts because the repository does not contain all three September Production migration sources; a separate approved deployment operation must recheck the live ledger and exact file hashes before applying them. Freeze the reviewed draft hashes, matching code SHA, Draft PR, and Production smoke sequence. Production DB writes and deploy remain separate Owner decisions.

## Owner identity disposition and final rollout order

The Owner confirmed on 2026-09-26 that the one Production owner-profile without a verified Auth binding is unused and may remain unable to log in during this release. No identity mapping is created by this candidate. If that account becomes active, verify its exact Auth UUID and business profile UUID out of band and request separate approval before using the fail-closed binding template.

After the final disposable CI run passes, retain the tested SQL as review drafts with exact SHA-256 hashes. The future Production operation requires separate Owner approval and must run in this order: inspect current catalog and migration ledger; pause new provisioning; apply the reviewed RLS and server-provisioning drafts in the agreed transaction sequence; verify grants, policies, Auth mapping, and old-RPC revocation; deploy the exact matching MyBiz code SHA; smoke public store/menu/table/order/inquiry/onboarding and merchant order/payment/account access; then close the MyBiz gate. If the code cannot be promoted safely, stop new writes and use the data-preserving write-lockdown draft. Never restore anonymous CRUD automatically.
