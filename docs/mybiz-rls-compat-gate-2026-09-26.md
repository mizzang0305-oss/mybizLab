...
4. Local Windows has no Docker/Supabase CLI. GitHub Actions run [36211278158](https://github.com/mizzang0305-oss/mybizLab/actions/runs/36211278158) started disposable Supabase, applied the draft, passed 21 pgTAP assertions, denied anon `orders` GET/POST/PATCH/DELETE over the Data API with HTTP 401, passed 901 application tests, exercised the real public-store and merchant-orders handlers against synthetic persisted rows, verified onboarding request persistence and exact-row cleanup, checked merchant order-event cross-store denial and own-store persistence, and proved the write-lockdown draft preserved its synthetic row. The runner destroyed the stack afterward. This fixture is synthetic and does not replace a full Production-schema compatibility test.
