...
4. Local Windows has no Docker/Supabase CLI. GitHub Actions run [36212016533](https://github.com/mizzang0305-oss/mybizLab/actions/runs/36212016533) started disposable Supabase, applied the draft, passed 21 pgTAP assertions, denied anon `orders` GET/POST/PATCH/DELETE over the Data API with HTTP 401, passed 901 application tests, exercised public-store/menu/table, public-order persistence, merchant-orders, onboarding request persistence, merchant order-event cross-store denial and own-store persistence, exact synthetic cleanup, and data-preserving write lockdown. The runner destroyed the stack afterward. This fixture is synthetic and does not replace a full Production-schema compatibility test.
