...
- `store_setup_requests.created_by` is nullable in the exact schema. Its two existing own-row policies are dormant while RLS is off. The traced in-repo path supports a **server-only draft posture**: the draft keeps those policies but removes browser grants after enabling RLS. This remains subject to route and outside-consumer proof, not an Owner-approved Production decision. A future owner-scope design is separate.
