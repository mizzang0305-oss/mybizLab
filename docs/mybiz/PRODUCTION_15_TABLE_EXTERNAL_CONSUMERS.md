...
- `vercel.json`: repository rewrites present; no repository-declared cron schedule. Connected Vercel project listing identified `mybizlab`, but detailed project/function configuration retrieval did not succeed; an independent project-level function/cron inventory was not obtained. This is not proof of no Vercel consumer.
