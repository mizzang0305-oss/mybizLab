...
- [GitHub local-stack run 35837477773](https://github.com/mizzang0305-oss/mybizLab/actions/runs/35837477773) used CLI 2.117.0 and a disposable Ubuntu Docker Supabase project with PostgreSQL, GoTrue, PostgREST, and Kong. It used only synthetic Auth identities and fixture rows. Two clean runs each passed 255 Data API checks, including actual JWT role mapping, 15-table anon deny, own/cross/non-member RLS, service-role grants, payment-field browser UPDATE deny, and local rollback. The direct authenticated provisioning RPC also created a synthetic VIP store in both runs: **P0 bypass confirmed in the local implementation**. The service-role-only direct RPC was denied because `auth.uid()` was absent.
...
- The real application handler network-wrapper test in `mybiz-15-table-fullstack-routes.test.ts` exercised public snapshot/order, onboarding setup-request, merchant order-event authorization, the paid provisioning gate, and a diagnostic of the currently broken free provisioning path: 5/5 baseline and 6/6 candidate in each clean run. The free route returned `AUTHENTICATION_REQUIRED` through the service-role RPC, so its test passing records a **blocked behavior**, not business-route success. This is the actual handler over loopback HTTP, not a Vercel deployment or a full Production-schema replay.
- Local exact-head application validation: `npm ci`, lint, typecheck, and build passed; the full Vitest suite reported **898 passed, 6 skipped** (the local-stack-only integration tests skip without ephemeral credentials); `npm audit --omit=dev` reported **0 vulnerabilities**. `npm ci` did report two advisories when dev dependencies were included, and Vite emitted pre-existing chunk/dynamic-import warnings.
...
| `/api/stores/provision` | POST | server payment/plan gate | service_role RPC | `create_store_with_owner`, then target home/priority writes through definer | paid-no-payment 400; direct authenticated RPC bypass confirmed; free server path returned 500 `AUTHENTICATION_REQUIRED` | server gate cannot be bypassed and authorized free path works | **BLOCKED P0** |
...
| Local Supabase Security Advisor | executed after candidate in both runs; warning/error classification was not exported | EXECUTED_UNCLASSIFIED, not PASS |
...
Production Apply stays blocked even if the local candidate matrix is green. The separate RPC boundary repair must address the direct authenticated call **and** restore a working authorized server provisioning path before any 15-table Apply approval is reconsidered. External consumer and remaining route evidence is also incomplete.
