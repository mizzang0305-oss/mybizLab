...
|---|---|---|---|---|
| `public.create_store_with_owner(text × 9)` | `SECURITY DEFINER`; `authenticated` and `service_role` EXECUTE, `anon` denied; `auth.uid()` check; source inserts `public.stores`; no payment/billing predicate in function definition | `/api/stores/provision` validates plan and requires payment verification for paid plan, then calls RPC | authenticated local Auth JWT called `/rest/v1/rpc/create_store_with_owner` with `p_plan=vip`; synthetic store with VIP plan created twice without server route or payment proof ([run 35836940777](https://github.com/mizzang0305-oss/mybizLab/actions/runs/35836940777)) | **BLOCKER** |
