  ...
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="AI manager"
        title="AI 점장"
        description="오늘 주문, 매출, 인기/부진 메뉴, 단골 증가, 예약/웨이팅을 분석해 Gemini 또는 fallback 요약을 보여줍니다."
        actions={
      ...
      <Panel title="AI 조언 카드" subtitle={`응답 소스: ${data?.insightSource || 'fallback'}`}>
        <div className="rounded-[28px] bg-slate-950 p-6 text-white">
          <p className="text-sm leading-7 text-slate-200">{data?.insight}</p>
          {data?.insightError ? <p className="mt-4 text-xs text-amber-300">Fallback reason: {data.insightError}</p> : null}
        </div>
      </Panel>
    </div>
  );
