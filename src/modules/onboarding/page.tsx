...
import { ensureUniqueStoreSlug, normalizeStoreSlug } from '@/shared/lib/storeSlug';
...
const steps: Array<{ key: OnboardingStep; label: string; desc: string }> = [
  ...
  { key: 'result', label: '진단 결과', desc: '운영 점수와 전략 확인' },
...
const planCards = [
  { code: 'starter' as const, title: 'Starter', desc: '한 매장을 빠르게 시작하는 기본 플랜', features: ['AI 진단', '기본 매출 분석', '주문 관리'] },
  { code: 'pro' as const, title: 'Pro', desc: '고객 관리와 예약 운영까지 함께 보는 추천 플랜', features: ['고객 관리', '예약 관리', 'AI 운영 리포트'] },
  { code: 'business' as const, title: 'Business', desc: '운영 자동화와 리포트를 깊게 보는 확장 플랜', features: ['주간 운영 리포트', '통합 운영 분석', '브랜드 확장 준비'] },
];
...
function reviewNotes(flow: OnboardingFlowState) {
  ...
  return [`AI 운영 점수 ${flow.diagnosisResult.score}점`, `요약: ${flow.diagnosisResult.summary}`, ...flow.diagnosisResult.recommendedStrategies].join('\n');
}
...
export function OnboardingPage() {
  ...
  function submitDiagnosis() {
    const diagnosisResult = buildDiagnosisResult(flow.diagnosisInput);
    setFlow((current) => ({
      ...current,
      diagnosisResult,
      requestDraft: { ...buildRequestDraftFromDiagnosis(current.diagnosisInput), ...current.requestDraft, businessType: current.diagnosisInput.businessType, region: current.diagnosisInput.region },
  ...
  async function startCheckout() {
    try {
      ...
      const { payment } = await launchPortOneCheckout(flow.selectedPlan, {
        ...
        customData: { requestId: flow.requestId, step: 'onboarding', storeName: flow.requestDraft.storeName.trim() },
  ...
  const requestValid = ['storeName', 'ownerName', 'phone', 'email'].every((field) => flow.requestDraft[field as keyof typeof flow.requestDraft].trim());
  ...
  return (
    <div className="page-shell space-y-8 py-10 sm:py-14">
      <PageHeader
        eyebrow="스토어 AI 온보딩"
        title="AI 진단부터 결제, 스토어 생성, 대시보드 진입까지 한 번에 진행합니다"
        description="매장 상황을 입력하면 AI가 운영 진단을 정리하고, 스토어 생성 요청과 구독 결제, 승인, 관리자 대시보드 준비까지 순서대로 이어집니다."
        actions={
          <>
            <Link className="btn-secondary" to="/pricing">요금제 자세히 보기</Link>
            <button className="btn-secondary" onClick={() => { clearOnboardingFlowState(); setFlow(createInitialOnboardingFlowState()); setMessage(null); }} type="button">처음부터 다시</button>
          </>
        }
      />

      <Panel title="온보딩 진행 단계" subtitle="지금 단계와 다음 단계가 한눈에 보이도록 구성했습니다.">
      ...
      <div className="grid gap-8 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-6">
          {flow.step === 'diagnosis' ? (
            <Panel title="1. 스토어 AI 진단" subtitle="업종, 지역, 고객 유형, 운영 고민을 입력하면 AI 진단 결과를 바로 확인할 수 있습니다.">
              <div className="grid gap-5 md:grid-cols-2">
                <label><span className="field-label">업종</span><select className="input-base" onChange={(event) => updateDiagnosis('businessType', event.target.value)} value={flow.diagnosisInput.businessType}>{businessTypes.map((item) => <option key={item}>{item}</option>)}</select></label>
                <label><span className="field-label">지역</span><input className="input-base" onChange={(event) => updateDiagnosis('region', event.target.value)} placeholder="예: 서울 성수동" value={flow.diagnosisInput.region} /></label>
                <label className="md:col-span-2"><span className="field-label">고객 유형</span><input className="input-base" onChange={(event) => updateDiagnosis('customerType', event.target.value)} placeholder="예: 직장인 점심 고객과 재방문 고객" value={flow.diagnosisInput.customerType} /></label>
                <label className="md:col-span-2"><span className="field-label">운영 고민</span><textarea className="input-base min-h-36 resize-y" onChange={(event) => updateDiagnosis('operatingConcerns', event.target.value)} placeholder="예: 예약은 들어오는데 대기 관리가 어렵고, 재방문 고객 관리도 잘 안 됩니다." value={flow.diagnosisInput.operatingConcerns} /></label>
              </div>
              <div className="mt-6"><button className="btn-primary" disabled={!diagnosisValid} onClick={submitDiagnosis} type="button">AI 진단 결과 보기</button></div>
            </Panel>
          ) : null}

          {flow.step === 'result' && flow.diagnosisResult ? (
            <Panel title="2. 스토어 AI 진단 결과" subtitle="운영 점수와 추천 전략을 확인한 뒤 스토어 생성 요청으로 이어집니다.">
              <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                <div className="rounded-[32px] bg-slate-950 p-6 text-white"><p className="text-sm font-semibold text-orange-200">운영 점수</p><p className="mt-3 font-display text-5xl font-black">{flow.diagnosisResult.score}점</p><p className="mt-4 text-sm leading-6 text-slate-300">{flow.diagnosisResult.summary}</p></div>
                <div className="grid gap-4">
                  <div className="rounded-3xl border border-slate-200 bg-white p-5"><p className="text-sm font-semibold text-slate-500">추천 전략</p><div className="mt-3 space-y-3">{flow.diagnosisResult.recommendedStrategies.map((item) => <div key={item} className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-700">{item}</div>)}</div></div>
                  <div className="rounded-3xl border border-slate-200 bg-white p-5"><p className="text-sm font-semibold text-slate-500">매출 개선 포인트</p><div className="mt-3 space-y-3">{flow.diagnosisResult.revenueOpportunities.map((item) => <div key={item} className="rounded-2xl bg-orange-50 px-4 py-3 text-sm text-orange-800">{item}</div>)}</div></div>
                </div>
              </div>
              <div className="mt-6 flex flex-wrap gap-3"><button className="btn-primary" onClick={() => setFlow((current) => ({ ...current, step: 'request' }))} type="button">스토어 생성 요청 계속</button><button className="btn-secondary" onClick={() => setFlow((current) => ({ ...current, step: 'diagnosis' }))} type="button">진단 내용 수정</button></div>
            </Panel>
          ) : null}
            ...
            <Panel title="3. 스토어 생성 요청" subtitle="관리자 대시보드와 스토어 생성을 위해 필요한 기본 정보를 입력합니다.">
              <div className="grid gap-5 md:grid-cols-2">
                {[
                  ...
                  <label key={field}><span className="field-label">{label}</span><input className="input-base" onChange={(event) => updateRequest(field as keyof OnboardingFlowState['requestDraft'], event.target.value)} placeholder={placeholder} type={field === 'email' ? 'email' : 'text'} value={flow.requestDraft[field as keyof OnboardingFlowState['requestDraft']]} /></label>
                ))}
              </div>
              <div className="mt-5 rounded-3xl bg-slate-50 p-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">스토어 주소 미리보기</p><p className="mt-2 text-sm font-semibold text-slate-900">/{slugPreview}</p><p className="mt-2 text-sm text-slate-500">결제 후 이 주소를 기준으로 스토어와 관리자 대시보드가 연결됩니다.</p></div>
              <div className="mt-6 flex flex-wrap gap-3"><button className="btn-primary" disabled={!requestValid || submitRequest.isPending} onClick={() => void submitRequest.mutateAsync()} type="button">스토어 생성 요청 제출</button><button className="btn-secondary" onClick={() => setFlow((current) => ({ ...current, step: 'result' }))} type="button">이전 단계로</button></div>
            </Panel>
          ) : null}
                ...
                {planCards.map((plan) => {
                  ...
                  return (
                    <button key={plan.code} className={`rounded-3xl border p-5 text-left ${selected ? 'border-orange-300 bg-orange-50 shadow-[0_20px_45px_-30px_rgba(236,91,19,0.45)]' : 'border-slate-200 bg-white'}`} onClick={() => setFlow((current) => ({ ...current, selectedPlan: plan.code }))} type="button">
                      <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-orange-600">{plan.title}</p><p className="mt-3 font-display text-3xl font-black text-slate-900">월 {details.amount.toLocaleString()}원</p></div>{recommended ? <span className="rounded-full bg-slate-950 px-3 py-1 text-xs font-bold text-white">추천</span> : null}</div>
                      <p className="mt-4 text-sm leading-6 text-slate-600">{plan.desc}</p>
                      <div className="mt-5 space-y-2">{plan.features.map((item) => <div key={item} className="rounded-2xl bg-slate-50 px-3 py-2 text-sm text-slate-700">{item}</div>)}</div>
                    </button>
                  );
                })}
              </div>
              <div className="mt-6 flex flex-wrap gap-3"><button className="btn-primary" disabled={!flow.requestId || flow.paymentStatus === 'processing' || activateStore.isPending} onClick={() => void startCheckout()} type="button">{flow.paymentStatus === 'processing' ? '결제창 준비 중...' : 'PortOne 결제 진행'}</button><button className="btn-secondary" onClick={() => setFlow((current) => ({ ...current, step: 'request' }))} type="button">요청 정보 수정</button></div>
            </Panel>
          ) : null}
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={String(label)} className="flex items-center justify-between rounded-3xl border border-slate-200 bg-white px-5 py-4">
                    <div><p className="font-semibold text-slate-900">{label}</p><p className="mt-1 text-sm text-slate-500">{done ? '완료' : pending ? '진행 중' : '대기 중'}</p></div>
                    <span className={`rounded-full px-3 py-1 text-xs font-bold ${done ? 'bg-emerald-100 text-emerald-700' : pending ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500'}`}>{done ? '완료' : pending ? '처리 중' : '대기'}</span>
                  </div>
                ))}
              </div>
              {flow.activationStatus === 'completed' ? <div className="mt-6"><button className="btn-primary" onClick={() => navigate('/dashboard', { replace: true })} type="button">관리자 대시보드 바로 열기</button></div> : null}
            </Panel>
          ) : null}
        </div>
        ...
        <div className="space-y-6">
          <Panel title="현재 요약" subtitle="진단 결과와 신청 정보를 오른쪽에서 바로 확인할 수 있습니다.">
            <div className="space-y-4">
              <div className="rounded-3xl bg-slate-950 p-5 text-white"><p className="text-sm font-semibold text-orange-200">권장 플랜</p><p className="mt-2 font-display text-3xl font-black">{planCards.find((plan) => plan.code === flow.selectedPlan)?.title || 'Starter'}</p><p className="mt-2 text-sm text-slate-300">{flow.diagnosisResult ? 'AI 진단 기준으로 가장 적합한 플랜을 선택했습니다.' : 'AI 진단 결과를 기준으로 권장 플랜이 자동 제안됩니다.'}</p></div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5"><p className="text-sm font-semibold text-slate-500">진단 입력</p><div className="mt-3 space-y-2 text-sm text-slate-700"><p>업종: {flow.diagnosisInput.businessType}</p><p>지역: {flow.diagnosisInput.region}</p><p>고객 유형: {flow.diagnosisInput.customerType}</p></div></div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5"><p className="text-sm font-semibold text-slate-500">스토어 생성 준비</p><div className="mt-3 space-y-2 text-sm text-slate-700"><p>스토어명: {flow.requestDraft.storeName || '아직 입력 전'}</p><p>관리자 이메일: {flow.requestDraft.email || '아직 입력 전'}</p><p>스토어 주소: /{slugPreview}</p></div></div>
              {suggestionLabels.length ? <div className="rounded-3xl border border-slate-200 bg-white p-5"><p className="text-sm font-semibold text-slate-500">AI 추천 기능</p><div className="mt-3 flex flex-wrap gap-2">{suggestionLabels.map((item) => <span key={item} className="rounded-full bg-orange-100 px-3 py-2 text-xs font-semibold text-orange-700">{item}</span>)}</div></div> : null}
            </div>
          </Panel>
          <Panel title="다음 단계 안내" subtitle="지금 단계에서 무엇이 이어지는지 짧게 안내합니다.">
            <div className="space-y-3 text-sm leading-6 text-slate-600">
              {flow.step === 'diagnosis' ? <p>운영 고민을 입력하면 AI가 운영 점수, 추천 전략, 매출 개선 포인트를 정리합니다.</p> : null}
