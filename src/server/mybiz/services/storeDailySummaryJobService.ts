...
export async function handleAdminStoreDailySummaryJobsRequest(
  request: CustomerMemoryRequestLike,
  ...
  try {
    ...
    const jobRunId = normalizeText(url.searchParams.get('jobRunId') || url.pathname.split('/').filter(Boolean).at(-1));
    const data = await listStoreDailySummaryJobRuns({
      jobRunId: jobRunId && jobRunId !== 'background-jobs' && jobRunId !== 'daily-store-summary' ? jobRunId : undefined,
      repository: access.repository,
      runDate: normalizeText(url.searchParams.get('runDate')) || undefined,
      storeId,
    });

    if (jobRunId && jobRunId !== 'background-jobs' && jobRunId !== 'daily-store-summary') {
      const item = data.items.find((candidate) => candidate.jobRunId === jobRunId);
      return item
        ? json({ ok: true, data: item })
        : json({ ok: false, error: 'Background job run could not be found for this store.' }, 404);
    }

    return json({ ok: true, data });
  } catch (error) {
    return json({ ok: false, error: error instanceof Error ? error.message : 'Unknown background job read error.' }, 500);
  }
}
