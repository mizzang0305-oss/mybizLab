...
import { listInquiryInboxReadModel } from '../../../shared/lib/services/inquiryInboxReadModelService';
import type { InquiryStatus } from '../../../shared/types/models';
...
function normalizeStatus(value: string | null): InquiryStatus | undefined {
  return value === 'new' || value === 'in_progress' || value === 'completed' || value === 'on_hold'
    ? value
    : undefined;
}
...
export async function handleAdminCustomerMemoryInboxRequest(
  request: CustomerMemoryRequestLike,
  ...
  try {
    ...
    const data = await listInquiryInboxReadModel({
      repository: access.repository,
      status: normalizeStatus(url.searchParams.get('status')),
      storeId,
    });
    const inquiryId = normalizeText(url.searchParams.get('inquiryId') || url.pathname.split('/').filter(Boolean).at(-1));
    if (inquiryId && inquiryId !== 'inbox') {
      const item = data.items.find((candidate) => candidate.id === inquiryId);
      return item
        ? json({ ok: true, data: item })
        : json({ ok: false, error: 'Inquiry inbox item could not be found for this store.' }, 404);
    }

    return json({ ok: true, data });
  } catch (error) {
    return json({ ok: false, error: error instanceof Error ? error.message : 'Unknown inquiry inbox read error.' }, 500);
  }
}
