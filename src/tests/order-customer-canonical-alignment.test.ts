...
describe('order/customer canonical alignment', () => {
  ...
  it('documents a manual dry-run backfill that never overwrites existing orders.customer_id', () => {
    ...
    expect(runbook).not.toMatch(/order_customer_backfill_snapshot/i);
    expect(runbook).not.toMatch(/\bcx\.(id|customer_id)\b/i);
    expect(runbook).not.toMatch(/\bcte\.(metadata|payload)\b/i);
    expect(runbook).not.toMatch(/\bpe\.raw\b/i);
    expect(runbook).toContain('to_jsonb(cx)');
    ...
    expect(runbook).toMatch(/updated_orders\s+as\s*\(\s*update public\.orders/i);
    expect(runbook).toMatch(/returning\s+o\.order_id::text/i);
