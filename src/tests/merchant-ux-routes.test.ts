...
describe('merchant UX routes', () => {
  ...
  it('renders YouTube social foundation with disabled connection copy and scopes', async () => {
    ...
    expect(socialHtml).toContain('Threads publish adapter 준비');
    expect(socialHtml).toContain('Threads 발행은 게시 권한, 점주 승인, 토큰 암호화, connected 계정이 모두 준비된 서버 작업에서만 실행됩니다.');
    expect(socialHtml).toContain('Threads 발행');
