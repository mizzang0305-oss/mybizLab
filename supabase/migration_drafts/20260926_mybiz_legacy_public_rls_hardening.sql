...
-- 3) Merchant/browser tables: authenticated users are constrained by store membership.
...
grant select, insert, update, delete on table public.store_tables to authenticated;
...
using (
  case
    when store_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      then public.is_store_member(store_id::uuid)
    else false
  end
)
with check (
...
grant select, insert, update, delete on table public.store_priority_settings to authenticated;
...
grant select, insert, update, delete on table public.store_daily_metrics to authenticated;
...
grant select, insert, update, delete on table public.store_home_content to authenticated;

-- 4) Store setup requests are written by the server-side onboarding API.
-- Authenticated users may only inspect/update their own request.
...
grant select, update on table public.store_setup_requests to authenticated;

-- 5) Server-only / quarantined legacy tables intentionally receive no anon or
-- authenticated policy/grant: sessions, events, store_staff, store_modules,
-- ai_briefing_logs, store_analytics_profile.
