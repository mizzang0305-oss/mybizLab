...
insert into auth.users(id, email, raw_user_meta_data) values
  ('$member_id', 'foundation-member@example.invalid', '{}'::jsonb),
  ('$other_member_id', 'foundation-outsider@example.invalid', '{}'::jsonb);
insert into public.profiles(id, full_name, email) values
  ('$member_id', 'Foundation Member', 'foundation-member@example.invalid'),
  ('$other_member_id', 'Foundation Outsider', 'foundation-outsider@example.invalid');
insert into public.stores(store_id, name, slug) values
  ('$store_id', 'Foundation Store', 'foundation-atomic-store');
insert into public.store_members(store_id, profile_id, role) values
  ('$store_id', '$member_id', 'owner');
insert into public.service_jobs(id, store_id, vertical, service_name, requires_contract, contract_state, state, created_by)
values ('$job_id', '$store_id', 'cleaning', 'Foundation contention test', false, 'NOT_REQUIRED', 'WORK_COMPLETED', '$member_id');
SQL
...
select private.create_next_job_evidence_revision('$job_id', '$member_id', 'concurrent-foundation-ci');
...
if "${psql_base[@]}" -v VERBOSITY=verbose -c "set role service_role; select private.create_next_job_evidence_revision('$job_id', '$other_member_id', 'unauthorized');" >"$tmp_dir/deny.out" 2>&1; then
  echo 'REVISION_CROSS_STORE_DENY failed: unauthorized actor was accepted' >&2
  exit 1
fi
grep -q '42501' "$tmp_dir/deny.out"
